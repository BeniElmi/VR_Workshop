...
{
	UE_LOG(LogTemp, Display, TEXT("[Avatars] Request Spec Arrived [%llu]"), message->oculusUserID);
...
{ 
	UE_LOG(LogTemp, Display, TEXT("[Avatars] Loaded Texture: [%llu] - [%s]"), id, *TextureFormatToString(data->format));
...
UTexture2D* FOvrAvatarManager::LoadTexture(const ovrAvatarTextureAssetData* data)
{
	...
	default:
		UE_LOG(LogTemp, Warning, TEXT("[Avatars] Unknown pixel format [%u]."), (int)data->format);
		break;
	}
