...
#include "Private/TrackingState.h"

float DebugLineScale = 100.f;
bool DrawDebug = false;

FString UOvrAvatar::HandNames[ovrpHand_Count] = { FString("hand_left"), FString("hand_right") };
...
void UOvrAvatar::BeginPlay()
{
	...
	OvrAvatarHelpers::OvrAvatarHandISZero(HandInputState[ovrpHand_Left]);
	OvrAvatarHelpers::OvrAvatarHandISZero(HandInputState[ovrpHand_Right]);
	HandInputState[ovrpHand_Left].isActive = true;
	HandInputState[ovrpHand_Right].isActive = true;
...
void UOvrAvatar::UpdateTransforms(float DeltaTime)
{
	...
	if (IHeadMountedDisplayModule::IsAvailable())
	{
		...
		if (TrackingState.Update())
		{
			TrackingState.HeadPoseToAvatarTransform(BodyTransform);
			BodyTransform.position.y += PlayerHeightOffset;
		}

		TrackingState.HandPoseToAvatarTransform(HandInputState[ovrpHand_Left].transform, ovrpHand_Left);
		TrackingState.HandPoseToAvatarTransform(HandInputState[ovrpHand_Right].transform, ovrpHand_Right);

		HandInputState[ovrpHand_Right].transform.position.y += PlayerHeightOffset;
		HandInputState[ovrpHand_Left].transform.position.y += PlayerHeightOffset;

		ovrAvatarPose_UpdateBody(Avatar, BodyTransform);
		ovrAvatarPose_UpdateHands(Avatar, HandInputState[ovrpHand_Left], HandInputState[ovrpHand_Right]);
	}
}
...
void UOvrAvatar::UpdateSkeleton(UPoseableMeshComponent& mesh, const ovrAvatarSkinnedMeshPose& pose)
{
	...
	for (uint32_t BoneIndex = 0; BoneIndex < pose.jointCount; BoneIndex++)
	{
...
USceneComponent* UOvrAvatar::DetachHand(ovrpHand hand)
{
	USceneComponent* handComponent = nullptr;

	if (hand >= ovrpHand_Count || AvatarHands[hand].IsValid())
...
void UOvrAvatar::ReAttachHand(ovrpHand hand)
{
	if (hand < ovrpHand_Count && AvatarHands[hand].IsValid() && !RootAvatarComponents.Find(HandNames[hand]))
	{
...
void UOvrAvatar::SetCustomGesture(ovrpHand hand, ovrAvatarTransform* joints, uint32_t numJoints)
{
	if (!Avatar)
		return;
...
void UOvrAvatar::SetControllerVisibility(ovrpHand hand, bool visible)
{
	if (!Avatar)
		return;
