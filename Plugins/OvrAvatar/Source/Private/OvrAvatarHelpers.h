...
namespace OvrAvatarHelpers
{
	...
	inline void OvrPoseStatefIdentity(ovrpPoseStatef& poseState)
	{
		OvrPosefIdentity(poseState.Pose);
		OvrVector3fZero(poseState.AngularVelocity);
		OvrVector3fZero(poseState.Velocity);
		OvrVector3fZero(poseState.AngularAcceleration);
		OvrVector3fZero(poseState.Acceleration);
		poseState.Time = 0.0f;
	}
	...
	* inline void OvrTrackingStateIdentity(ovrTrackingState& state)
	{
		...
		OvrPoseStatefIdentity(state.HandPoses[ovrpHand_Left]);
		OvrPoseStatefIdentity(state.HandPoses[ovrpHand_Right]);

		state.StatusFlags = 0;
		state.HandStatusFlags[ovrpHand_Left] = state.HandStatusFlags[ovrpHand_Right] = 0;
	} */
